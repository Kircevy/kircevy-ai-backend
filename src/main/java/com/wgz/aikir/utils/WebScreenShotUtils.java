...
public class WebScreenShotUtils {
    ...
    static {
        ...
        log.info("初始化Chrome浏览器（线程：{}）", Thread.currentThread().getName());
    ...
    private void destroy() {
        ...
        if (driver != null) {
            log.info("关闭Chrome浏览器（线程：{}）", Thread.currentThread().getName());
            driver.quit();
            webDriverThreadLocal.remove(); // 避免内存泄漏
        }
    }
    ...
    private static WebDriver getWebDriver() {
        ...
        if (driver == null) {
            final int DEFAULT_WIDTH = 1600;
            final int DEFAULT_HEIGHT = 900;
            log.info("延迟初始化Chrome浏览器（线程：{}）", Thread.currentThread().getName());
